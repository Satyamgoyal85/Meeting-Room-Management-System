...
export default function ResetPasswordForm({ employeeName }: ResetPasswordFormProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      ...
      // On success, resetPasswordAction calls redirect() server-side,
      // so no client-side navigation needed here
    });
  };
      ...
      <div className="glass-panel rounded-3xl p-8 shadow-2xl relative overflow-hidden">
        ...
        <div className="text-center mb-8 relative z-10">
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-2 font-medium leading-relaxed">
            Welcome, <span className="text-emerald-600 dark:text-emerald-400 font-bold">{employeeName}</span>! 
            <br />
        ...
        <div className="mb-6 p-3.5 bg-sky-50 dark:bg-sky-950/50 border border-sky-200 dark:border-sky-900/60 rounded-xl text-xs text-sky-700 dark:text-sky-300 flex items-start space-x-2.5">
          ...
          <span>
            This is a one-time setup. After this, use your new password every time you log in.
          </span>
        </div>
