...
export default function Navbar({ session }: NavbarProps) {
  ...
  const isReceptionistPage = pathname === '/receptionist';
    ...
    <header className="sticky top-0 z-50 bg-white/90 dark:bg-slate-900/90 backdrop-blur-md border-b border-slate-200 dark:border-slate-800 shadow-sm transition-all">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
        ...
        <div className="flex items-center space-x-6">
          <Link 
            href={isAdmin ? "/admin" : isReceptionist ? "/receptionist" : "/dashboard"} 
          ...
          >
            ...
            <div>
              ...
              <span className="text-[10px] sm:text-xs block text-slate-500 dark:text-slate-400 font-medium">
                GHO Branch • {isAdmin ? 'Admin Portal' : isReceptionist ? 'Receptionist Portal' : 'Room Dashboard'}
              </span>
            </div>
          </Link>
          ...
          <nav className="hidden md:flex items-center space-x-1 lg:space-x-1.5 pl-4 lg:pl-6 border-l border-slate-200 dark:border-slate-800">
            {/* 1. Calendar (Admin -> /admin?tab=calendar, Receptionist -> /receptionist?tab=calendar, Employee -> /dashboard/calendar) */}
            {isAdmin ? (
              <Link
              ...
              <Link
                href="/receptionist?tab=calendar"
                title="Calendar"
                className={`px-2.5 py-1.5 rounded-lg text-xs font-semibold flex items-center space-x-1.5 transition-colors ${
                  isReceptionistPage && adminTab === 'calendar'
            ...
            {/* 4. All Bookings (Available to Admin & Receptionist) */}
            {(isAdmin || isReceptionist) && (
              <Link
                href={isAdmin ? "/admin?tab=bookings" : "/receptionist?tab=bookings"}
                className={`px-2.5 py-1.5 rounded-lg text-xs font-semibold flex items-center space-x-1.5 transition-colors ${
                  (isAdminPage || isReceptionistPage) && adminTab === 'bookings'
                    ? isAdmin
                      ? 'bg-purple-50 text-purple-700 dark:bg-purple-950/60 dark:text-purple-300'
                      : 'bg-blue-50 text-blue-700 dark:bg-blue-950/60 dark:text-blue-300'
                    : 'text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800'
            ...
            {/* 5-6. Admin only links (Rooms & Depts, Reports) */}
            {isAdmin && (
              <>
                <Link
        ...
        <div className="hidden md:flex items-center space-x-3 relative shrink-0">
          ...
          >
            <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold text-white shadow-sm shrink-0 ${
              isAdmin ? 'bg-purple-600' : isReceptionist ? 'bg-blue-600' : 'bg-emerald-600'
            }`}>
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden border-t border-slate-200 dark:border-slate-800 bg-white/95 dark:bg-slate-900/95 backdrop-blur-md px-4 pt-3 pb-6 space-y-4 animate-in slide-in-from-top duration-200">
          ...
          <nav className="space-y-1.5">
            ...
            {/* 4. All Bookings (Mobile for Admin or Receptionist) */}
            {(isAdmin || isReceptionist) && (
              <div className="pt-2 mt-2 border-t border-slate-200 dark:border-slate-800 space-y-1.5">
                <div className={`px-4 text-[10px] font-bold uppercase tracking-wider ${isAdmin ? 'text-purple-600 dark:text-purple-400' : 'text-blue-600 dark:text-blue-400'}`}>
                  {isAdmin ? 'Admin Control Portal' : 'Receptionist Portal'}
                </div>
                ...
                >
                  <LayoutList className={`w-5 h-5 ${isAdmin ? 'text-purple-600' : 'text-blue-600'}`} />
